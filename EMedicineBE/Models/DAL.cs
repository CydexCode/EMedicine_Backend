...
namespace EMedicineBE.Models
{
    public class DAL
    {
        public Response register(Users users,SqlConnection connection)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_rejister", connection);
        ...
        public Response login(Users users,SqlConnection connection)
        {
          ...
          SqlDataAdapter da = new SqlDataAdapter("sp_login",connection);
            ...
            if(dt.Rows.Count>0)
            {
               ...
               response.StatusCode=100;
                response.StatusMessage = "User is Inavalid";
                response.user = null;
            }
        ...
        public Response updateProfile(Users users,SqlConnection connection)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_upadeteProfile", connection);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@FirstName", users.FirstName);
            cmd.Parameters.AddWithValue("@LastName", users.LastName);
            cmd.Parameters.AddWithValue("@Password", users.Password);
           ...
           connection.Open();
            ...
            {
                response.StatusCode = 100;
                response.StatusMessage = "some error occcured . Try after sometime";
            }
            return response;
        }
        ...
        public Response addToCart(Cart cart,SqlConnection connection) {
            ...
            {
                response.StatusCode = 200;
                response.StatusMessage = "Item added successfuly";
            }
            else
            {
                response.StatusCode = 100;
        ...
        public Response placeOrder(Users users,SqlConnection connection)
        {
            ...
            {
                response.StatusCode = 200;
                response.StatusMessage = "Order has been placed successfuly";
            }
            else
            {
                response.StatusCode = 100;
        ...
        public Response addUpdateMedicine(Medicines medicines,SqlConnection connection)
        {
            ...
            {
                response.StatusCode = 200;
                response.StatusMessage = "Medicine inserted successfuly";
            }
            else
            {
                response.StatusCode = 100;
                response.StatusMessage = "Medicine didi not save.Try again";
            }
        ...
        public Response userList(SqlConnection connection)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    response.StatusCode = 200;
                    response.StatusMessage = "Userr details fetched";
                    response.listUsers = listUsers;
                }
                else
                {
                    response.StatusCode = 100;
                    response.StatusMessage = "Userr details are not available";
